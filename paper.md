...
The output likelihood is computed for a given set of models and parameters from the solution of the spherical Jeans' equation (see e.g. Ref. \cite{MamLok05}),
\begin{equation}
\label{eq:sigmajeans}
\sigma^2_r(r)=\frac{1}{\nu(r)}\int_r^{\infty}{\exp\left[2\int_r^s{\frac{\beta(t)}{t}\text{d}t}\right]\nu(s)\frac{\text{d}\Phi}{\text{d}s}\text{d}s},
\end{equation}
projected in the phase space. The above equation captures the main necessary input required for the MG-MAMPOSSt method. In particular, the gradient of the gravitational potential, which in turn contains information about the model of gravity, as well as the velocity anisotropy profile ($\beta(r)$) and the projected number density of tracers ($\nu(r)$).

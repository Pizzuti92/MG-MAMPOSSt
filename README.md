...
```bash
 Best-fit from optimization
   ...
   r_mass   =  0.358 (c_mass=      3.93)
   Anisotropy parameter =   1.5018
   First MG parameter =  0.0000
   Second MG parameter =  0.0000
 Likelihood =   5893.90604
 ```
 The plot of the marginalized distributions of the free parameters generated by the test run should correspond to what is shown by [this figure](https://github.com/Pizzuti92/MG-MAMPOSSt/blob/main/test/plot_example_test1.png). 
...
Now, we define the details of the run by working on the parameters in pars_test.txt. In particular, we choose to work with the two free parameters defining the modified gravity model, which are the background value of the chameleon field **\phi_\infty**, given in unit of 1e-5 and the coupling constant **Q**. This means that the only numbers different from zero in the first block should be the last two:
```bash
0. 
0.
0.

...
C     The array WB will be used for working space. Its length must be at least
...
C     requires the first (NPT+2)*(NPT+N)+3*N*(N+5)/2 elements of WB plus the
...
C
   ...
   20     FORMAT (/4X,'Return from BOBYQA because one of the',
     1      ' differences XU(I)-XL(I)'/6X,' is less than 2*RHOBEG.')
      ...
      WB(JSL)=XL(J)-X(J)
      WB(JSU)=XU(J)-X(J)
      IF (WB(JSL) .GE. -RHOBEG) THEN
          IF (WB(JSL) .GE. ZERO) THEN
              X(J)=XL(J)
              WB(JSL)=ZERO
              WB(JSU)=TEMP
          ELSE
              X(J)=XL(J)+RHOBEG
              WB(JSL)=-RHOBEG
              WB(JSU)=DMAX1(XU(J)-X(J),RHOBEG)
          END IF
      ELSE IF (WB(JSU) .LE. RHOBEG) THEN
          IF (WB(JSU) .LE. ZERO) THEN
              X(J)=XU(J)
              WB(JSL)=-TEMP
              WB(JSU)=ZERO
          ELSE
              X(J)=XU(J)-RHOBEG
              WB(JSL)=DMIN1(XL(J)-X(J),-RHOBEG)
              WB(JSU)=RHOBEG
          END IF
      END IF
   30 CONTINUE
...
C
      CALL BOBYQB (N,NPT,X,XL,XU,RHOBEG,RHOEND,IPRINT,MAXFUN,WB(IXB),
     1  WB(IXP),WB(IFV),WB(IXO),WB(IGO),WB(IHQ),WB(IPQ),
     2  WB(IBMAT),WB(IZMAT),
     3  NDIM,WB(ISL),WB(ISU),WB(IXN),WB(IXA),WB(ID),WB(IVL),WB(IW))
   40 RETURN
      END
...
C       in order to increase the denominator in the updating of UPDATEB.
...
C     WB is a one-dimensional array that is used for working space. Its length
...
C
   60 CALL TRSBOX (N,NPT,XPT,XOPT,GOPT,HQ,PQ,SL,SU,DELTA,XNEW,D,
     1  WB,WB(NP),WB(NP+N),WB(NP+2*N),WB(NP+3*N),DSQ,CRVMIN)
...
C
     ...
     1       GOTO 650
          ...
          DO 80 J=1,N
          BDTEST=BDTOL
          IF (XNEW(J) .EQ. SL(J)) BDTEST=WB(J)
          IF (XNEW(J) .EQ. SU(J)) BDTEST=-WB(J)
...
C
  ...
  100     SUM=SUM+XPT(K,I)*XOPT(I)
          ...
          IP=NPT+I
          DO 110 J=1,I
  110     BMAT(IP,J)=BMAT(IP,J)+WB(I)*VLAG(J)+VLAG(I)*WB(J)
C
...
C
  ...
  140     BMAT(K,J)=BMAT(K,J)+SUM*ZMAT(K,JJ)
          ...
          TEMP=WB(I)
          DO 150 J=1,I
  150     BMAT(IP,J)=BMAT(IP,J)+TEMP*WB(J)
C
...
C
          IH=0
          DO 170 J=1,N
          WB(J)=-HALF*SUMPQ*XOPT(J)
          DO 160 K=1,NPT
          WB(J)=WB(J)+PQ(K)*XPT(K,J)
  160     XPT(K,J)=XPT(K,J)-XOPT(J)
          DO 170 I=1,J
          IH=IH+1
          HQ(IH)=HQ(IH)+WB(I)*XOPT(J)+XOPT(I)*WB(J)
...
C
  210 CALL ALTMOV (N,NPT,XPT,XOPT,BMAT,ZMAT,NDIM,SL,SU,KOPT,
     1  KNEW,ADELT,XNEW,XALT,ALPHA,CAUCHY,WB,WB(NP),WB(NDIM+1))
      DO 220 I=1,N
  220 D(I)=XNEW(I)-XOPT(I)
C
C     Calculate VLAG and BETA for the current choice of D. The scalar
C     product of D with XPT(K,.) is going to be held in WB(NPT+K) for
...
C
  ...
  410 VQUAD=VQUAD+HQ(IH)*TEMP
      DO 420 K=1,NPT
  420 VQUAD=VQUAD+HALF*PQ(K)*WB(NPT+K)**2
      DIFF=F-FOPT-VQUAD
      DIFFC=DIFFB
      DIFFB=DIFFA
      DIFFA=DABS(DIFF)
...
C
      CALL UPDATEB (N,NPT,BMAT,ZMAT,NDIM,VLAG,BETA,DENOM,KNEW,WB)
      IH=0
      PQOLD=PQ(KNEW)
      PQ(KNEW)=ZERO
      DO 470 I=1,N
...
C
  ...
  510 SUMB=SUMB+XPT(K,J)*XOPT(J)
      TEMP=SUMA*SUMB
      DO 520 I=1,N
  520 WB(I)=WB(I)+TEMP*XPT(K,I)
      DO 530 I=1,N
  530 GOPT(I)=GOPT(I)+DIFF*WB(I)
C
...
C
  ...
  610     WB(K)=SUM*WB(K)
          ...
          SUM=ZERO
          DO 620 K=1,NPT
  620     SUM=SUM+BMAT(K,I)*VLAG(K)+XPT(K,I)*WB(K)
...
C
  ...
  740     FORMAT (/4X,'At the return from BOBYQA',5X,
     ...
     1  KNEW,ADELT,XNEW,XALT,ALPHA,CAUCHY,GLAG,HCOL,WB)
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION XPT(NPT,*),XOPT(*),BMAT(NDIM,*),ZMAT(NPT,*),SL(*),
     1  SU(*),XNEW(*),XALT(*),GLAG(*),HCOL(*),WB(*)
...
C       UPDATEB. The step XNEW-XOPT from XOPT is restricted to moves along the
...
C     WB is a working space vector of length 2N that is going to hold the
...
C
C     Set the first NPT components of WB to the leading elements of the
...
C     step in W. The sum of squares of the fixed components of WB is formed in
C     WFIXSQ, and the free components of WB are set to BIGSTP.
C
      BIGSTP=ADELT+ADELT
      IFLAG=0
  100 WFIXSQ=ZERO
  ...
  110 CONTINUE
      ...
      END IF
C
C     Investigate whether more components of WB can be fixed.
C
  120 TEMP=ADELT*ADELT-WFIXSQ
      IF (TEMP .GT. ZERO) THEN
          ...
          GGFREE=ZERO
          DO 130 I=1,N
          IF (WB(I) .EQ. BIGSTP) THEN
              TEMP=XOPT(I)-STEP*GLAG(I)
              IF (TEMP .LE. SL(I)) THEN
                  WB(I)=SL(I)-XOPT(I)
                  WFIXSQ=WFIXSQ+WB(I)**2
  ...
  130     CONTINUE
      ...
      END IF
C
C     Set the remaining free components of WB and all components of XALT,
C     except that WB may be scaled later.
C
      GW=ZERO
      DO 140 I=1,N
      IF (WB(I) .EQ. BIGSTP) THEN
          WB(I)=-STEP*GLAG(I)
          XALT(I)=DMAX1(SL(I),DMIN1(SU(I),XOPT(I)+WB(I)))
      ELSE IF (WB(I) .EQ. ZERO) THEN
          XALT(I)=XOPT(I)
      ...
      ELSE
          XALT(I)=SU(I)
      END IF
  140 GW=GW+GLAG(I)*WB(I)
C
C     Set CURV to the curvature of the KNEW-th Lagrange function along W.
C     Scale WB by a factor less than one if that can reduce the modulus of
...
C       p=0, respectively.
C     The first NDIM+NPT elements of the array WB are used for working space. 
...
C
      ...
      WB(J)=HALF*SUMPQ*XOPT(J)
      DO 30 K=1,NPT
   30 WB(J)=WB(J)+PQ(K)*XPT(K,J)
      DO 40 I=1,J
      IH=IH+1
   40 HQ(IH)=HQ(IH)+WB(I)*XOPT(J)+WB(J)*XOPT(I)
C
...
C     The nonnegative values of WB(NDIM+K) are required in the search below.
C
          CALL UPDATEB (N,NPT,BMAT,ZMAT,NDIM,VLAG,BETA,DENOM,KNEW,WB)
          IF (NREM .EQ. 0) GOTO 350
          DO 110 K=1,NPT
  110     WB(NDIM+K)=DABS(WB(NDIM+K))
      END IF
C
...
C
  120 DSQMIN=ZERO
      DO 130 K=1,NPT
      IF (WB(NDIM+K) .GT. ZERO) THEN
          IF (DSQMIN .EQ. ZERO .OR. WB(NDIM+K) .LT. DSQMIN) THEN
              KNEW=K
              DSQMIN=WB(NDIM+K)
          END IF
      END IF
...
C
  ...
  140 WB(NPT+J)=XPT(KNEW,J)
      ...
      ELSE IF (PTSID(K) .EQ. ZERO) THEN
          DO 150 J=1,N
  150     SUM=SUM+WB(NPT+J)*XPT(K,J)
      ELSE
          IP=PTSID(K)
          IF (IP .GT. 0) SUM=WB(NPT+IP)*PTSAUX(1,IP)
          IQ=DFLOAT(NP)*PTSID(K)-DFLOAT(IP*NP)
          IF (IQ .GT. 0) THEN
              IW=1
              IF (IP .EQ. 0) IW=2
              SUM=SUM+WB(NPT+IQ)*PTSAUX(IW,IQ)
          END IF
      END IF
  160 WB(K)=HALF*SUM*SUM
C
...
C
      ...
      SUM=ZERO
      DO 170 J=1,N
  170 SUM=SUM+BMAT(K,J)*WB(NPT+J)
  180 VLAG(K)=SUM
      BETA=ZERO
      DO 200 J=1,NPTM
      SUM=ZERO
      DO 190 K=1,NPT
  190 SUM=SUM+ZMAT(K,J)*WB(K)
      BETA=BETA-SUM*SUM
      DO 200 K=1,NPT
  200 VLAG(K)=VLAG(K)+SUM*ZMAT(K,J)
      ...
      SUM=ZERO
      DO 210 K=1,NPT
  210 SUM=SUM+BMAT(K,J)*WB(K)
      JP=J+NPT
      BSUM=BSUM+SUM*WB(JP)
      DO 220 IP=NPT+1,NDIM
  220 SUM=SUM+BMAT(IP,J)*WB(IP)
      BSUM=BSUM+SUM*WB(JP)
      VLAG(JP)=SUM
...
C     of the denominator in the updating calculation of UPDATEB.
C
      DENOM=ZERO
      VLMXSQ=ZERO
      DO 250 K=1,NPT
  ...
  250 VLMXSQ=DMAX1(VLMXSQ,VLAG(K)**2)
      IF (DENOM .LE. 1.0D-2*VLMXSQ) THEN
          WB(NDIM+KNEW)=-WB(NDIM+KNEW)-WINC
          GOTO 120
      END IF
      GOTO 80
C
...
C
      DO 290 I=1,N
      WB(I)=DMIN1(DMAX1(XL(I),XBASE(I)+XPT(KPT,I)),XU(I))
      IF (XPT(KPT,I) .EQ. SL(I)) WB(I)=XL(I)
...
C
  ...
  260 HRED(I)=HS(I)
     ...
     1  KNEW,WB)
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION BMAT(NDIM,*),ZMAT(NPT,*),VLAG(*),WB(*)
...
C     at most ZTEST. The first NDIM elements of WB are used for working space.
C
C     Set some constants.
C
      ONE=1.0D0
...
C
      ...
      JP=NPT+J
      WB(JP)=BMAT(KNEW,J)
      TEMPA=(ALPHA*VLAG(JP)-TAU*WB(JP))/DENOM
      TEMPB=(-BETA*WB(JP)-TAU*VLAG(JP))/DENOM
      DO 60 I=1,JP
      BMAT(I,J)=BMAT(I,J)+TEMPA*VLAG(I)+TEMPB*WB(I)

	     real*8 function lfdeviate2(alpha,xmin,xmax,seed)
c
c LFDEVIATE generates random x = L/L* from Schechter LF
c from xmin L* to xmax L*.
c
c compile with -lnr
c
        implicit none
...
c
        ...
        lfdeviate2 = -1.d0
        do while (lfdeviate2 .lt. 0.d0)
            q = r_uniform_01(seed)
            lfdeviate2 = (q*(xmax**alpha1-xmin**alpha1) 
     &		 + xmin**alpha1)**(1.d0/alpha1)
c
            q = r_uniform_01(seed)
            if (q .gt. exp(-lfdeviate2)) then
                lfdeviate2 = -1.d0
            endif
        enddo
c
